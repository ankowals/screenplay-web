...
class MediaAttacher {
    ...
    void attachMedia(Map<FqdnTestName, ExtentTest> testStatusMap) {
        ...
        testStatusMap.forEach((fqdnTestName, extentTest) -> {
            screenshots.stream()
                    ...
                    .ifPresent(screenshot -> extentTest.addScreenCaptureFromPath("./" + screenshot.getName()));
    ...
    void attachedNestedMedia(Map<FqdnTestName, ExtentTest> testStatusMap) {
        ...
        testStatusMap.forEach((fqdnTestName, extentTest) -> {
            Optional<File> maybeMediaDirForTestClass = dirs.stream()
                    .filter(dir -> fqdnTestName.getClassName().equals(dir.getName()))
                    .findFirst();

            maybeMediaDirForTestClass.ifPresent(dir -> {
                List<File> screenshots = this.mediaFinder.getScreenshots(dir);

                screenshots.stream()
                        .filter(this.fileNamePrefixEquals(fqdnTestName.getMethodName()))
                        .max(Comparator.comparingLong(File::lastModified))
                        .ifPresent(screenshot -> extentTest.addScreenCaptureFromPath("./" + maybeMediaDirForTestClass.get().getName() + "/" + screenshot.getName()));
                ...
                videos.stream()
                        ...
                        .ifPresent(video -> {
                            switch (extentTest.getStatus()) {
                                case FAIL:
                                    extentTest.fail(MarkupHelper.toTable(new MyVideo("./" + maybeMediaDirForTestClass.get().getName() + "/" + video.getName())));
                                    break;
                                case PASS:
                                    extentTest.pass(MarkupHelper.toTable(new MyVideo("./" + maybeMediaDirForTestClass.get().getName() + "/" + video.getName())));
                                    break;
                            }
                        });
            });
    ...
    private Predicate<File> fileNamePrefixEquals(String expectedPrefix) {
        return file -> {
            Optional<String> maybeValidInfix = Stream.of("_arg0_", "_driver_")
                    .filter(infix -> file.getName().contains(infix))
                    .findFirst();

            return maybeValidInfix.filter(infix -> file.getName()
                    .substring(0, file.getName().indexOf(infix))
